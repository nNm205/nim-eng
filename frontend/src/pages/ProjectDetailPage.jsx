...
import {
  ArrowLeft, Trash2, Archive, ArchiveRestore, Edit2, Calendar,
  CheckCircle2, AlertCircle, FileText, BarChart3, ClipboardList,
...
const ProjectDetailsPage = () => {
  ...
  const handleArchive = async () => {
    ...
    try {
      ...
      setProject(updated);
      setMessage({ type: "success", text: project.is_archived ? "Bỏ lưu trữ dự án thành công!" : "Lưu trữ dự án thành công!" });
      setTimeout(() => setMessage({ type: "", text: "" }), 3000);
    } catch (err) {
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        ...
        {message.text && (
          <div className={`border rounded-xl px-6 py-4 flex items-start gap-3 ${message.type === "success" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-red-50 border-red-200 text-red-700"}`}>
        ...
        {!isEditing ? (
          <div className="space-y-8">
            <div className="flex items-start justify-between">
              ...
              {project.is_archived && (
                <span className="flex-shrink-0 bg-slate-100 text-slate-600 text-xs px-3 py-1 rounded-lg font-medium">📦 Đã lưu trữ</span>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                {project.description && (
                  <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
                    <h2 className="text-xl font-bold text-slate-900 mb-4 flex items-center gap-2">
                      <span className="w-1 h-1 rounded-full bg-teal-600" />Mô tả
                    </h2>
                ...
                {project.research_scope && (
                  <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
                    <h2 className="text-xl font-bold text-slate-900 mb-4 flex items-center gap-2">
                      <span className="w-1 h-1 rounded-full bg-teal-600" />Phạm vi nghiên cứu
                    </h2>
                    <p className="text-slate-700 whitespace-pre-wrap leading-relaxed">{project.research_scope}</p>
                  </div>
                )}

                {/* Stats Grid */}
                <div className="grid grid-cols-3 gap-4 bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
                  <StatItem icon={FileText} value={documents.length} label="Tài liệu" />
                  <StatItem icon={BarChart3} value={project.research_session_count ?? 0} label="Phân tích" />
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                      ...
                      Tài liệu
                      <span className="text-base font-normal text-slate-400">({documents.length})</span>
                    </h2>
                  ...
                  {documents.length === 0 ? (
                    <div className="text-center py-10">
                      ...
                      <p className="text-slate-400 text-sm mt-1">Dùng chức năng Tìm kiếm để thêm tài liệu vào dự án</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      ...
                      {documents.length > 5 && (
                        <button onClick={() => navigate(`/projects/${projectId}/documents`)}
                          ...
                          Xem thêm {documents.length - 5} tài liệu
                        </button>
                      )}
                    </div>
                  )}
              ...
              <div className="space-y-6">
                <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="border-t border-slate-200 pt-4">
                      ...
                      <p className="text-sm text-slate-700 font-medium">{formatDate(project.created_at)}</p>
                    </div>
                    <div className="border-t border-slate-200 pt-4">
                      <p className="text-xs font-semibold text-slate-600 mb-2 flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4" />Cập nhật lần cuối
                      </p>
                      <p className="text-sm text-slate-700 font-medium">{formatDate(project.updated_at)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm space-y-3">
                  ...
                  <button onClick={handleArchive} disabled={apiLoading}
                    ...
                    {project.is_archived ? <><ArchiveRestore className="w-4 h-4" />Bỏ lưu trữ</> : <><Archive className="w-4 h-4" />Lưu trữ</>}
                  </button>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-sm font-semibold text-slate-900 mb-2">Tên dự án <span className="text-red-500">*</span></label>
                <input type="text" name="name" required minLength={3} maxLength={255} value={formData.name} onChange={handleChange}
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-900 mb-2">Chủ đề</label>
                <input type="text" name="topic" maxLength={500} value={formData.topic} onChange={handleChange}
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 focus:border-transparent transition-all" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-900 mb-2">Trạng thái</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-900 mb-2">Phạm vi nghiên cứu</label>
                <textarea name="research_scope" rows={5} value={formData.research_scope} onChange={handleChange}
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 focus:border-transparent transition-all resize-none" />
              </div>
...
const DocumentRow = ({ doc, onClick }) => {
  const sourceTypeColors = {
    ...
    uploaded: "text-teal-600 bg-teal-50 border-teal-200",
  };
  const colorClass = sourceTypeColors[doc.source_type] || sourceTypeColors.uploaded;

  return (
    <div onClick={onClick}
      className="flex items-center gap-4 p-4 rounded-xl border border-slate-200 hover:border-teal-300 hover:bg-teal-50/30 cursor-pointer transition-all group">
      <div className={`flex-shrink-0 p-2 rounded-lg border ${colorClass}`}>
        <FileText className="w-4 h-4" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-slate-900 truncate group-hover:text-teal-700 transition-colors">{doc.title}</p>
