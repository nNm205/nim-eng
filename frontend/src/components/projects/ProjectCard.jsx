  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md hover:border-teal-300 transition-all group">
      ...
      <div className="flex items-start justify-between mb-4">
        <button onClick={onClick} className="flex-1 text-left group hover:text-teal-600">
      ...
      {/* Stats — dùng document_count, research_session_count, report_count từ backend */}
      <div className="flex items-center gap-4 mb-4 pb-4 border-b border-slate-200">
        <button onClick={onClick}
          className="flex items-center gap-1.5 text-xs text-slate-600 hover:text-teal-600 transition-colors"
          title="Tài liệu">
          <FileText className="w-4 h-4" />
          <span className="font-semibold">{project.document_count ?? 0}</span>
        </button>
        <button onClick={onClick}
          className="flex items-center gap-1.5 text-xs text-slate-600 hover:text-teal-600 transition-colors"
          title="Phiên nghiên cứu">
          <BarChart3 className="w-4 h-4" />
          <span className="font-semibold">{project.research_session_count ?? 0}</span>
        </button>
        <button onClick={onClick}
          className="flex items-center gap-1.5 text-xs text-slate-600 hover:text-teal-600 transition-colors"
          title="Báo cáo">
          <ClipboardList className="w-4 h-4" />
          <span className="font-semibold">{project.report_count ?? 0}</span>
        </button>
      </div>
